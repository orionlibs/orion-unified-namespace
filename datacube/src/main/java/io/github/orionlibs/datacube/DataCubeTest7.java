...
public class DataCubeTest7
{
    public static void main(String[] args) throws Exception
    {
        ...
        SingleOutputStreamOperator<SpotlightDataRow> aggregatedStream1 = keyedByOpExCategory.reduce((row1, row2) -> {
            row1.movement += row2.movement;
            return row1;
        });
        System.out.println("\n=== Grouped by OpExCategory ===");
        try(CloseableIterator<SpotlightDataRow> resultIterator = aggregatedStream1.executeAndCollect())
        {
        ...
        KeyedStream<SpotlightDataRow, String> keyedByDepartment = aggregatedStream1.keyBy(record -> record.department);
        SingleOutputStreamOperator<SpotlightDataRow> aggregatedStream2 = keyedByOpExCategory.reduce((row1, row2) -> {
            row1.movement += row2.movement;
            return row1;
        });
        System.out.println("\n=== Grouped by OpExCategory -> Department ===");
        try(CloseableIterator<SpotlightDataRow> resultIterator = aggregatedStream2.executeAndCollect())
        {
            while(resultIterator.hasNext())
            {
                System.out.println(resultIterator.next());
        ...
        SingleOutputStreamOperator<SpotlightDataRow> aggregatedStream3 = keyedByOpExCategory.reduce((row1, row2) -> {
            row1.movement += row2.movement;
            return row1;
        });
        System.out.println("\n=== Grouped by OpExCategory -> Department -> EMS Level 1 ===");
        try(CloseableIterator<SpotlightDataRow> resultIterator = aggregatedStream3.executeAndCollect())
        {
            while(resultIterator.hasNext())
            {
                System.out.println(resultIterator.next());
